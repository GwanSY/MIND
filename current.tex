...
\toprule
\textbf{Notations} & \textbf{Description} \\ \midrule
...
We use the MNIST dataset\cite{xiao2017fashion}, a widely recognized benchmark in machine learning and computer vision research. MNIST consists of 70,000 grayscale images of handwritten digits, including 60,000 training samples and 10,000 test samples. Each image is represented as a 28$\times$28 pixel matrix, where each pixel indicates the intensity of grayscale values ranging from 0 to 255. This dataset is widely used for developing and evaluating image classification models due to its simplicity and versatility. It serves as a standard for testing various preprocessing, feature extraction, and classification techniques, providing a consistent foundation for assessing model performance.
...
Table \ref{tab
} shows that when using Network-A for privacy-preserving secure inference on the MNIST dataset, MIND achieves an accuracy, precision, recall, and F1 score of 0.978, 0.978, 0.978, and 0.978, respectively.
In contrast, Pencil achieves an accuracy, precision, recall, and F1 score of 0.978, 0.978, 0.978, and 0.978, respectively.
The performance of the remaining networks is similar.
...
\hline
\multirow{8}{*}{MNIST}   & \multirow{2}{*}{ABY3}      & Pencil & 0.978 & 0.978 & 0.978 & 0.978 \\ 
                      &            & MIND    & 0.978 & 0.978 & 0.978 & 0.978 \\ 
\cline{2-7}
                      & \multirow{2}{*}{Chameleon}   & Pencil & 0.985 & 0.985 & 0.985 & 0.985 \\ 
                      &            & MIND    & 0.985 & 0.985 & 0.985 & 0.985 \\ 
\cline{2-7}
                      & \multirow{2}{*}{Sphinx}      & Pencil & 0.991 & 0.990 & 0.991 & 0.991 \\ 
                      &            & MIND    & 0.991 & 0.990 & 0.991 & 0.991 \\ 
\cline{2-7}
                      & \multirow{2}{*}{Quotient}   & Pencil & 0.980 & 0.980 & 0.980 & 0.980 \\ 
...
% 采用network-A时，MIND的通信开销和runtime分别为多少MB和多少seconds，对比Pencil通信开销减少 多少MB，效率提升几倍。在采用其他network时，MIND对比Pencil通信开销降低%，效率提升多少X。




Experiments demonstrate that MIND outperforms Pencil in terms of efficiency across various networks in MNIST dataset.
...
\begin{figure}[ht]
    \centering
    \subfigure[ABY3]{\includegraphics[width=.45\columnwidth]{exp/ABY3.pdf}\label{subfig:ABY3}} \hspace{5pt}
    ...
    \subfigure[Chameleon]{\includegraphics[width=.45\columnwidth]{exp/Chameleon.pdf}\label{subfig:Chameleon}} \hspace{5pt}\\
    \subfigure[Sphinx]{\includegraphics[width=.45\columnwidth]{exp/Sphinx.pdf}\label{subfig:Sphinx}} 
