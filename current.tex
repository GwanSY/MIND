...
\begin{itemize}
    \item Network-A.
    Network-A is a fully connected deep neural network (DNN) designed for MNIST classification, implemented in a structure suitable for secure multi-party computation using the ABY3 protocol \cite{10.1145/3243734.3243760}. It starts with a Flatten layer converting 1×28×28 input into a vector of size 784. The first Linear layer maps size 784 to 128 with ReLU, followed by another Linear layer mapping size 128 to 128 with ReLU. The final Linear layer maps size 128 to 10, representing class probabilities.
    \item Network-B.
    Network-B is a privacy-preserving convolutional neural network (CNN) implemented under the Chameleon protocol \cite{10.1145/3196494.3196522}.  Network-B is a 3-layer CNN for MNIST classification. It begins with a Conv2d layer (input: 1×28×28, kernel: 5×5, stride: 2, padding: 2) producing 5 output channels (5×14×14), followed by ReLU. A Flatten layer converts the feature map into a vector of size 980. A Linear layer maps this to size 100, followed by ReLU, and another Linear layer maps it to size 10 for classification probabilities.
    \item Network-C.
    Network-C is a privacy-preserving convolutional neural network (CNN) implemented under the Sphinx protocol \cite{Tian2022SphinxEP}.
    ...
    \item Network-D.
    Network-D is a privacy-preserving convolutional neural network (DNN) implemented under the Quotient protocol \cite{10.1145/3319535.3339819}.
