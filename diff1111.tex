...
%DIF DEL 1001.tex   Mon Nov 11 06:51:34 2024
%DIF ADD 1111.tex   Mon Nov 11 07:01:12 2024
...
\begin{abstract}
Image processing and analysis capabilities have been revolutionized by the rapid advancement of deep learning technologies. Model inference enables powerful image analytics, however, complex neural networks often require more computational resources than end \DIFdelbegin \DIFdel{device }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend can provide. While cloud \DIFdelbegin \DIFdel{server }\DIFdelend \DIFaddbegin \DIFadd{servers }\DIFaddend help alleviate the resource constraints of these devices, transferring sensitive image data between \DIFdelbegin \DIFdel{end device }\DIFdelend \DIFaddbegin \DIFadd{the end devices }\DIFaddend and the cloud server introduces high computational \DIFdelbegin \DIFdel{cost }\DIFdelend \DIFaddbegin \DIFadd{costs }\DIFaddend and significant privacy risks. In addition, traditional collaborative inference encryption methods usually involve directly encrypting image data, but such methods often fail to effectively balance security and performance. To address this issue, we propose MIND, a novel cloud-based collaborative encryption method \DIFdelbegin \DIFdel{designed to enhance both }\DIFdelend \DIFaddbegin \DIFadd{to enhance }\DIFaddend privacy protection and computational efficiency. MIND divides the encryption process between the end \DIFdelbegin \DIFdel{device }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend and the cloud server, with the end \DIFdelbegin \DIFdel{device }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend encrypting part of the model parameters and the cloud server encrypting the image data and remaining parameters. This approach leverages the strengths of \DIFdelbegin \DIFdel{both end device }\DIFdelend \DIFaddbegin \DIFadd{end devices }\DIFaddend and cloud resources to \DIFdelbegin \DIFdel{achieve a balance between }\DIFdelend \DIFaddbegin \DIFadd{balance }\DIFaddend security and performance. Our experimental results demonstrate that MIND effectively protects user privacy while maintaining high efficiency, successfully integrating end \DIFdelbegin \DIFdel{device }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend into the secure inference process. 
\end{abstract}
...
\DIFdelbegin \DIFdel{Artificial }\DIFdelend \DIFaddbegin \DIFadd{Advancements in artificial }\DIFaddend intelligence and cloud computing \DIFdelbegin \DIFdel{advancements }\DIFdelend have revolutionized image processing and analysis \DIFdelbegin \DIFdel{capabilities }\DIFdelend across various industries \cite{makridakis2017forthcoming} \cite{zhang2010cloud}. Model inference has become a critical component in cloud-based image processing and analysis, leveraging \DIFdelbegin \DIFdel{the power of }\DIFdelend deep learning models to extract valuable insights from visual data. \cite{shokri2017membership}. This approach enables sophisticated image interpretation across various domains, from medical diagnostics to autonomous systems. However, the widespread adoption of cloud-based model inference raises significant privacy concerns, as it often requires the transmission and processing of sensitive personal data \cite{shokri2017membership}\cite{wang2018stealing}. For instance, facial recognition systems in residential areas and financial institutions' use of image data for commercial purposes exemplify scenarios where user privacy may be compromised \cite{cui2018security}. These practices highlight the tension between leveraging advanced technologies and protecting individual privacy rights \cite{van2014datafication}.

\DIFdelbegin \DIFdel{The conventional approach to image analysis typically }\DIFdelend \DIFaddbegin \DIFadd{Conventionally, image analysis }\DIFaddend involves encrypting the entire image before uploading it to the cloud server. \DIFdelbegin \DIFdel{This method, however, }\DIFdelend \DIFaddbegin \DIFadd{However, this method }\DIFaddend results in significant storage and communication overhead due to the large size of encrypted images. Moreover, conducting image analysis during transmission presents considerable privacy protection challenges, as sensitive data must be securely handled to prevent unauthorized access \cite{cui2018security}\cite{van2014datafication}.

Existing privacy-preserving scheme for model inference attempt to address these concerns through various encryption techniques \cite{fu2020vfl}\cite{yang2012efficient}. Traditional encryption methods, while providing a degree of data security, often introduce substantial computational overhead and communication cost \cite{kerschbaum2012outsourced}. This inefficiency becomes particularly pronounced when dealing with large-scale data, hindering the practicality of such approaches in real-world scenarios \cite{li2015encdb}. Moreover, current solutions struggle to \DIFdelbegin \DIFdel{strike an optimal balance between }\DIFdelend \DIFaddbegin \DIFadd{balance }\DIFaddend privacy protection, computational efficiency, and communication overhead, especially in \DIFdelbegin \DIFdel{the context of }\DIFdelend end-cloud collaborative environments.
% \subsection{Contribution}

To address these limitations, we propose MIND, a novel privacy-preserving model inference framework enabled by end-cloud collaboration. MIND employs advanced technologies, including homomorphic encryption and secret sharing, to ensure the security of both data and model parameters during the inference process. By distributing computational tasks between end devices and cloud server, MIND maintains inference efficiency while enhancing data privacy and reducing communication overhead. Our framework introduces a layered encryption scheme that dynamically adjusts encryption methods based on the model \DIFdelbegin \DIFdel{'s structure, significantly optimizing both }\DIFdelend \DIFaddbegin \DIFadd{structure, optimizing }\DIFaddend computational and communication \DIFdelbegin \DIFdel{cost}\DIFdelend \DIFaddbegin \DIFadd{costs}\DIFaddend . The main contributions of our work are as follows:
...
As the demand for secure data processing in cloud computing environments increases, researchers have conducted various studies to improve the efficiency and privacy protection of collaborative inference methods between cloud \DIFdelbegin \DIFdel{server and end device}\DIFdelend \DIFaddbegin \DIFadd{servers and end devices}\DIFaddend . To address these challenges, researchers have proposed numerous solutions based on various technologies. We categorize the previous work into three main subcategories.

Federated learning (FL) is an innovative approach \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{to }\DIFaddend machine learning that prioritizes the privacy of training data. In horizontal FL (e.g., \cite{pmlr-v54-mcmahan17a}, \cite{Gupta2018DistributedLO}, \cite{9252066}), each client (i.e., data owner or end \DIFdelbegin \DIFdel{device}\DIFdelend \DIFaddbegin \DIFadd{devices}\DIFaddend ) independently trains a local model using its private data and then submits the obfuscated local models to a central model aggregator (i.e., model owner or cloud server). However, because the global model is synchronized during each training iteration, FL does not adequately address the privacy concerns of the global model. Furthermore, FL tends to disproportionately favor end \DIFdelbegin \DIFdel{device}\DIFdelend \DIFaddbegin \DIFadd{devices}\DIFaddend , as each end device gains access to the global model even if they contribute only a small portion of the training data. Consequently, when the server has an exclusive proprietary interest in the final model, FL is not an ideal solution.

To safeguard both data and model privacy, the research community has developed a range of distributed training methods centered on secure multiparty computation (MPC, e.g., \cite{7958569}, \cite{Escudero2020ImprovedPF}, \cite{Dalskov2020FantasticFH}). In these approaches, the typical scenario involves a server-assisted framework where a group of data owners \DIFdelbegin \DIFdel{upload }\DIFdelend \DIFaddbegin \DIFadd{uploads }\DIFaddend secret-shared versions of their private data to at least two independent, non-colluding third-party servers. These servers collaboratively train the model using the secret-shared data. During the training process, the model itself is also secret-shared across all servers, ensuring complete protection of model privacy. However, as previously mentioned, these approaches face a significant \DIFdelbegin \DIFdel{trade off }\DIFdelend \DIFaddbegin \DIFadd{trade-off }\DIFaddend between the requirement for non-collusion and the need for extensibility.

The third subcategory of research depends on homomorphic encryption (HE, e.g, \cite{Nandakumar2019TowardsDN}, \cite{CiC-1-2-22}). In cloud computing \cite{gentry2009fully}, which allows computations to be performed on encrypted data without the need for decryption. This approach makes it feasible to execute complex operations on cloud server while maintaining data privacy. However, the computational overhead associated with fully homomorphic encryption remains non-trivial when dealing with large-scale data \cite{xu2019cryptonn}. Although Cheon \textit{et al.} \cite{cheon2017homomorphic} have attempted to optimize homomorphic encryption scheme to reduce this overhead, the trade-off between computational efficiency and security continues to be a critical issue.

As research progresses, there's a growing interest in leveraging the strengths of both edge and cloud computing to enhance security and efficiency. To leverage the advantages of both edge and cloud computing, Mao \textit{et al.} \cite{mao2017survey} proposed an end-cloud collaborative model. While this approach shows promise, it imposes certain performance requirements on the end \DIFdelbegin \DIFdel{device }\DIFdelend \DIFaddbegin \DIFadd{devices }\DIFaddend and does not effectively address the issue of reducing communication overhead. In a related effort, Akshaya and Khadir \textit{et al.} \cite{kakkad2019biometric} explored approaches to enhance image security in cloud frameworks using biometric authentication and image encryption.
...
$pk_e$                                   & The public key of the end \DIFdelbeginFL \DIFdelFL{device}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{devices}\DIFaddendFL . \\
$pk_s$                                   & The public key of the cloud \DIFdelbeginFL \DIFdelFL{server}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{servers}\DIFaddendFL . \\
...
$\langle \cdot \rangle_i$                & The secret share between end \DIFdelbeginFL \DIFdelFL{device and cloud server}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{devices and cloud servers}\DIFaddendFL . \\
$r_i$                                    & Random vector generated by end \DIFdelbeginFL \DIFdelFL{device and cloud server}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{devices and cloud servers}\DIFaddendFL . \\
...
\multirow{2}{*}{$y_p$}                                    & The intermediate computed result after the encryption \\
                                               & of model parameters on the end \DIFdelbeginFL \DIFdelFL{device}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{devices}\DIFaddendFL .\\
$y_d$                                    & The data after encryption on the server side.\\
\DIFaddbeginFL \DIFaddFL{\circ            }&\DIFaddFL{The linear operator.}\\
\DIFaddFL{\oplus           }&\DIFaddFL{The addition of two encrypted values in the HE scheme.}\\
\DIFaddFL{\ominus          }&\DIFaddFL{The subtraction of two encrypted values in the HE scheme.}\\
\DIFaddFL{\otimes          }&\DIFaddFL{The multiplication of two encrypted values in the HE scheme.}\\
...
\begin{algorithm}[htbp]
	...
	\DIFdelbegin %DIFDELCMD < \KwOut{
%DIFDELCMD < 	

%DIFDELCMD < 	}
%DIFDELCMD <     %%%
\DIFdelend \DIFaddbegin \KwOut{
	The final inference result $y$
	}
    \DIFaddend $ED$ sets $\langle x_0 \rangle_1=x$ and  $S$ sets the $\langle x_0 \rangle_2 = 0$.
...
In the MIND experiments, we \DIFdelbegin \DIFdel{conduct comparison studies , evaluating }\DIFdelend \DIFaddbegin \DIFadd{perform comparative studies to evaluate }\DIFaddend the layered encryption method against the \DIFdelbegin \DIFdel{encryption }\DIFdelend method proposed in this paper. The evaluation criteria include accuracy, F1 score, recall, runtime, and communication cost.

\subsection{Setting}
\subsubsection{Environment} Our \DIFdelbegin \DIFdel{solution is implemented in python, and the experiments run }\DIFdelend \DIFaddbegin \DIFadd{experiments are conducted }\DIFaddend on two servers\DIFdelbegin \DIFdel{that both }\DIFdelend \DIFaddbegin \DIFadd{, each }\DIFaddend equipped with an AMD EPYC 7402 CPU and 128 GB of RAM.
\subsubsection{Datasets} 
We employ the \DIFdelbegin \DIFdel{widely-used MNISTdataset }\DIFdelend \DIFaddbegin \DIFadd{widely used MNIST}\DIFaddend \cite{xiao2017fashion} and CIFAR-10 \DIFdelbegin \DIFdel{dataset}\DIFdelend \DIFaddbegin \DIFadd{datasets}\DIFaddend \cite{krizhevsky2009learning} to train and evaluate our models. To ensure the data is suitable for model input, the following preprocessing steps are applied. The MNIST dataset \DIFdelbegin \DIFdel{comprises }\DIFdelend \DIFaddbegin \DIFadd{consists of }\DIFaddend 60,000 training images and 10,000 test images, each a 28$\times$28 pixel grayscale image of handwritten digits. We normalize the pixel values to the [0, 1] range and implemente data augmentation techniques such as random rotations and translations during training. The CIFAR-10 dataset consists of 50,000 training images and 10,000 test images, each a 32$\times$32 pixel color image categorize into 10 classes. We standardize \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend images so that \DIFdelbegin \DIFdel{the }\DIFdelend pixel values for each channel have zero mean and unit variance. We train each \DIFdelbegin \DIFdel{image neural network using }\DIFdelend \DIFaddbegin \DIFadd{neural network on }\DIFaddend the training dataset for 10 epochs. \DIFdelbegin \DIFdel{Additionally, we employ }\DIFdelend \DIFaddbegin \DIFadd{To further improve the model's generalization ability, we apply }\DIFaddend data augmentation techniques, including random cropping, horizontal flipping, and color jittering\DIFdelbegin \DIFdel{, to enhance the model's generalization capability}\DIFdelend .




\subsubsection{Parameters} 
The learning rate $\eta$ is set to 0.01 to accelerate model convergence while maintaining stability. For the MNIST dataset, a batch size $B$ of 32 is used, while a batch size $B$ of 64 is \DIFdelbegin \DIFdel{employed }\DIFdelend \DIFaddbegin \DIFadd{utilized }\DIFaddend for the CIFAR-10 dataset. The momentum $\gamma$ is configured at 0.8 to effectively smooth the update process and reduce gradient oscillations during training. Additionally, the gradient bound estimation $C$ is set to 8 to limit the \DIFdelbegin \DIFdel{maximum gradientvalue and prevent }\DIFdelend gradient\DIFaddbegin \DIFadd{'s maximum value, preventing gradient }\DIFaddend explosion.

\subsection{Effectiveness}
In this subsection, we evaluate the effectiveness of the scheme. The MIND scheme we propose employs \DIFdelbegin \DIFdel{hierarchical encryption}\DIFdelend \DIFaddbegin \DIFadd{a hierarchical encryption method}\DIFaddend , while the non-hierarchical encryption scheme is \DIFdelbegin \DIFdel{called }\DIFdelend \DIFaddbegin \DIFadd{referred to as the }\DIFaddend Naive scheme. We \DIFdelbegin \DIFdel{achieve evaluation by utilizing }\DIFdelend \DIFaddbegin \DIFadd{evaluate the performance using }\DIFaddend models with different architectures on the MNIST dataset, specifically MNIST ABY3\cite{10.1145/3243734.3243760}, MNIST Sphinx\cite{Tian2022SphinxEP}, MNIST Chameleon\cite{10.1145/3196494.3196522} and MNIST Quotient 2$\times$512\cite{10.1145/3319535.3339819}. On the CIFAR-10 dataset, we \DIFdelbegin \DIFdel{use }\DIFdelend \DIFaddbegin \DIFadd{employ }\DIFaddend AlexNet\cite{NIPS2012_c399862d} and ResNet50\cite{7780459}. \DIFdelbegin \DIFdel{We compare }\DIFdelend \DIFaddbegin \DIFadd{The evaluation focuses on comparing }\DIFaddend the accuracy, precision, recall, and F1 score between \DIFdelbegin \DIFdel{hierarchical encryption scheme }\DIFdelend \DIFaddbegin \DIFadd{the hierarchical }\DIFaddend and non-hierarchical encryption \DIFdelbegin \DIFdel{scheme based on }\DIFdelend \DIFaddbegin \DIFadd{schemes using }\DIFaddend these MNIST models.

Based on the results presented in Figure \ref{fig:ACC}, we observe no significant difference in recognition accuracy between the layered and non-layered encryption schemes. This demonstrates that our proposed MIND scheme effectively maintains model performance while implementing enhanced privacy protection measures.

\DIFdelbegin \DIFdel{As the data }\DIFdelend \DIFaddbegin \DIFadd{The data presented }\DIFaddend in Table \ref{tab:performance_comparison_scheme} \DIFdelbegin \DIFdel{shows, the performance of }\DIFdelend \DIFaddbegin \DIFadd{indicates that }\DIFaddend all tested models \DIFdelbegin \DIFdel{is consistent }\DIFdelend \DIFaddbegin \DIFadd{perform consistently }\DIFaddend in both the \DIFdelbegin \DIFdel{Native and MIND }\DIFdelend \DIFaddbegin \DIFadd{Naive and MIND schemes}\DIFaddend . In the MNIST task, models like ABY3, Chameleon, Sphinx, and Quotient show nearly identical precision, recall, and F1 scores in both environments. Similarly, AlexNet and ResNet50 maintain consistent performance in the CIFAR-10 task, albeit with lower overall scores due to the \DIFdelbegin \DIFdel{task's complexity }\DIFdelend \DIFaddbegin \DIFadd{greater complexity of the task}\DIFaddend .

It is worth noting that MIND does not significantly affect model performance, especially in simpler tasks. Even \DIFdelbegin \DIFdel{in }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend more complex tasks, the consistency between environments \DIFdelbegin \DIFdel{remains, although the overall performance decreases }\DIFdelend \DIFaddbegin \DIFadd{is maintained, although overall scores may decrease }\DIFaddend slightly. This suggests that the MIND is effective in achieving privacy preserving goals while maintaining model accuracy.
\begin{table}[ht]
\centering
...
To demonstrate the efficiency of MIND, in this subsection, we evaluate and compare the computational overhead of Native and MIND under different models. We take the average results of multiple experiments to reduce the bias caused by random noise.
